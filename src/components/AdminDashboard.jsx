...
export default function AdminDashboard() {
  ...
  useEffect(() => { localStorage.setItem("quickambu_sent_requests", JSON.stringify(sentRequests)); }, [sentRequests]);
  useEffect(() => { localStorage.setItem("quickambu_incoming_history", JSON.stringify(incomingRequestsHistory)); }, [incomingRequestsHistory]);
  ...
  useEffect(() => {
    ...
    if (isAuth !== "true") { navigate("/AdminLogin"); }
      ...
      try {
        ...
        if (response.data.success && response.data.data.length > 0) {
          setAmbulances(response.data.data.map((d, index) => ({ 
            id: d._id, driver: d.name, phone: d.mobile, vehicleNumber: `RJ 18 PA ${1000 + index}`, type: "ALS (ICU)", status: "Available", kmValue: 99999, distance: "Distance Pending", coords: rajasthanCoords[index % rajasthanCoords.length], 
            images: [
        ...
        } else { setAmbulances(initialAmbulances); }
      } catch (error) { setAmbulances(initialAmbulances); }
    };
    fetchAllDrivers();
  }, [navigate]);
  ...
  useEffect(() => {
    const checkMissedRequests = () => {
      ...
      if (missedReq && !currentLiveUser) {
        ...
        setCurrentLiveUser({
          id: parsedReq.id, name: parsedReq.name, phone: parsedReq.mobile, location: parsedReq.location || "Location Coordinates Synced", emergencyType: parsedReq.emergency, time: parsedReq.time, coords: parsedReq.coords 
        });
      }
    };
    checkMissedRequests();
  ...
  // 🚀🚀 THE GOD MODE FIX: BACKEND BYPASS 🚀🚀
  // ये बैकएंड का इंतज़ार ही नहीं करेगा, ड्राइवर के 'Trip Complete' दबाते ही 1 मिलीसेकंड में खुद एम्बुलेंस को आज़ाद कर देगा!
  useEffect(() => {
    const crossTabSync = (e) => {
      if (e.key === "force_free_ambulance" && e.newValue) {
        const data = JSON.parse(e.newValue);
        const finalStatus = data.status === "Rejected" ? "Rejected by Driver" : "Completed Trip";
        
        setSentRequests(prev => prev.map(req => req.id === data.reqId ? { ...req, status: finalStatus } : req));
        setIncomingRequestsHistory(prev => prev.map(req => req.id === data.reqId ? { ...req, status: finalStatus } : req));
        
        localStorage.removeItem("force_free_ambulance");
      }
    };
    window.addEventListener("storage", crossTabSync);
    return () => window.removeEventListener("storage", crossTabSync);
  }, []);

  useEffect(() => {
    socket.on("receive-request", (newReq) => {
      setCurrentLiveUser({ id: newReq.id, name: newReq.name, phone: newReq.mobile, location: newReq.location || "Live GPS Location", emergencyType: newReq.emergency, time: newReq.time, coords: newReq.coords });
    ...
    return () => {
      socket.off("receive-request"); socket.off("journey-status-updated"); socket.off("driver-location-update");
    };
  }, []);

  const getJourneyLabel = (step) => {
    if (step === -1) return "Rejected by Driver";
    const labels = ["Dispatched", "Ambulance Started", "On The Way", "Reached Patient", "Patient On Board", "Reached Hospital", "Completed Trip", "Completed Trip"];
    return labels[step] || "Completed Trip";
  };
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      setImgIndexes((prev) => {
        const updated = { ...prev };
        ambulances.forEach((amb) => { const currentIndex = prev[amb.id] || 0; updated[amb.id] = currentIndex === 0 ? 1 : 0; });
        return updated;
      });
    }, 4000);
  ...
  useEffect(() => {
    if (currentLiveUser) {
      startAlertTone();
      setPatientName(currentLiveUser.name || ""); setPatientPhone(currentLiveUser.phone || ""); setPatientEmergency(currentLiveUser.emergencyType || ""); setUserLocation(currentLiveUser.location || "");
      const isAlreadyLogged = incomingRequestsHistory.some(h => h.id === currentLiveUser.id);
      if (!isAlreadyLogged) {
        setIncomingRequestsHistory(prev => [{ id: currentLiveUser.id || `REQ-${Date.now().toString().slice(-4)}`, time: currentLiveUser.time || new Date().toLocaleTimeString(), name: currentLiveUser.name, phone: currentLiveUser.phone, location: currentLiveUser.location, status: "Pending Action" }, ...prev]);
      }
    } else { stopAlertTone(); }
    ...
    try {
      ...
      if (!oscillatorRef.current) {
        ...
        const modulationTimer = setInterval(() => {
          ...
          } else { clearInterval(modulationTimer); }
        }, 400);
  ...
  const dismissRequest = () => {
    localStorage.removeItem("global_pending_request"); setCurrentLiveUser(null); setIsLocationFiltered(false); stopAlertTone();
  };

  const handleLocationSearchFilter = () => {
    if (!currentLiveUser || !currentLiveUser.coords) {
      setCustomNotification({ show: true, title: "Location Error", message: "मरीज़ की लोकेशन नहीं मिल पा रही है!", type: "error" }); return;
    }
    const userLat = currentLiveUser.coords[0]; const userLng = currentLiveUser.coords[1];
    setAmbulances(prev => prev.map(amb => {
      if (amb.coords && amb.coords.length === 2) {
        const exactDistance = calculateDistance(userLat, userLng, amb.coords[0], amb.coords[1]);
        return { ...amb, kmValue: exactDistance, distance: `${exactDistance.toFixed(1)} km` };
      }
      return amb;
    }));
  ...
  const handleConfirmDeleteAmbulance = () => {
    if (enteredMasterKey === COMPANY_MASTER_KEY) {
      ...
      setDeleteTargetId(null); setEnteredMasterKey("");
    } else { setCustomNotification({ show: true, title: "Access Denied!", message: "Invalid Company Master Key! Action aborted.", type: "error" }); }
  };

  const displayedAmbulances = isLocationFiltered ? [...ambulances].sort((a, b) => a.kmValue - b.kmValue) : ambulances;

  const sendAmbulanceRequest = (ambId) => {
    if (!patientName || !patientPhone || !patientEmergency) {
      setCustomNotification({ show: true, title: "Incomplete Form", message: "Please fill details correctly!", type: "error" }); return;
    }
    ...
    const dispatchData = { 
      reqId: generatedReqId, driverId: amb.id, driverName: amb.driver, vehicle: amb.id, patientName: patientName, patientMobile: patientPhone, emergency: patientEmergency, location: userLocation, coords: [userLat, userLng] 
    };
    ...
    const newDispatch = {
      id: generatedReqId, ambId: ambId, vehicleNumber: amb.vehicleNumber, patientName: patientName, patientPhone: patientPhone, emergencyType: patientEmergency, location: userLocation, time: new Date().toLocaleTimeString(), status: "Dispatched", userLat: userLat, userLng: userLng, ambLat: ambulanceStartLat, ambLng: ambulanceStartLng, distanceKm: amb.distance, driverName: amb.driver, driverPhone: amb.phone
    };
    ...
    setOpenFormId(null); setCurrentLiveUser(null);
  };
  ...
  return (
    <div className={`${isDark ? 'dark' : ''}`}>
      <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-gray-900 dark:text-slate-100 font-sans antialiased transition-colors duration-300">
        ...
        <header className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border-b border-gray-200 dark:border-slate-800 h-20 flex items-center px-6 md:px-10 justify-between sticky top-0 z-50 shadow-sm">
          ...
          <div className="flex items-center gap-4 sm:gap-6">
            ...
            <button onClick={() => setIsDark(!isDark)} className="p-2.5 rounded-full bg-gray-100 dark:bg-slate-800 text-gray-600 dark:text-yellow-400 hover:bg-gray-200 dark:hover:bg-slate-700 transition-colors" title="Toggle Theme">
        ...
        {deleteTargetId && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-md">
            <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-2xl border border-gray-200 dark:border-slate-700 max-w-sm w-full p-6 space-y-4">
              ...
              <div className="text-center space-y-1"><h3 className="text-lg font-black text-gray-900 dark:text-white">Remove Vehicle</h3><p className="text-xs text-gray-500 dark:text-gray-400">Master Passkey required to delete <strong>{deleteTargetId}</strong>.</p></div>
        ...
        {customNotification.show && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
            <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl max-w-sm w-full p-6 text-center space-y-4 border dark:border-slate-800">
              ...
              <div><h4 className="text-lg font-bold text-gray-900 dark:text-white">{customNotification.title}</h4><p className="text-sm text-gray-600 dark:text-gray-400 mt-1">{customNotification.message}</p></div>
        ...
        <main className="max-w-7xl mx-auto px-4 py-8 space-y-8">
          ...
          <div className={`bg-white dark:bg-slate-900 border-2 rounded-3xl p-5 md:p-8 shadow-lg space-y-5 transition-all duration-500 relative overflow-hidden ${currentLiveUser ? "border-red-500 shadow-red-500/20" : "border-gray-200 dark:border-slate-800"}`}>
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b border-gray-100 dark:border-slate-800 pb-4 gap-4">
              ...
              <div className="flex items-center gap-3">
                {currentLiveUser && (
                  <button onClick={stopAlertTone} className="bg-red-50 dark:bg-red-900/20 hover:bg-red-100 dark:hover:bg-red-900/40 text-red-600 dark:text-red-400 border border-red-200 dark:border-red-800 font-bold px-4 py-2 rounded-xl text-xs md:text-sm flex items-center gap-2 transition-all cursor-pointer"><IoVolumeMute className="text-lg" /> Stop Siren</button>
                )}
                {currentLiveUser && ( <span className="bg-red-600 text-white font-mono text-[10px] md:text-xs px-3 py-1.5 rounded-full font-bold shadow-sm">Time: {currentLiveUser.time || "Live"}</span> )}
              </div>
            </div>
            ...
            {!currentLiveUser ? (
              <div className="bg-gray-50 dark:bg-slate-800/50 border border-dashed border-gray-300 dark:border-slate-700 rounded-2xl p-10 flex flex-col items-center justify-center text-center gap-3">
                ...
                <div><p className="text-gray-500 dark:text-slate-400 font-bold">No Active Emergency Requests</p><p className="text-xs text-gray-400 dark:text-slate-500 mt-1">Waiting for transmission from user devices...</p></div>
              </div>
            ) : (
              <>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-sm">
                  <div className="bg-gray-50 dark:bg-slate-800 p-4 rounded-2xl border border-gray-100 dark:border-slate-700"><span className="text-gray-400 dark:text-slate-500 font-bold uppercase tracking-wider text-[10px]">Patient Name</span><p className="font-extrabold text-gray-900 dark:text-white mt-1 text-base">{currentLiveUser.name}</p></div>
                  <div className="bg-gray-50 dark:bg-slate-800 p-4 rounded-2xl border border-gray-100 dark:border-slate-700"><span className="text-gray-400 dark:text-slate-500 font-bold uppercase tracking-wider text-[10px]">Mobile Number</span><p className="font-extrabold text-gray-900 dark:text-white mt-1 text-base">{currentLiveUser.phone}</p></div>
                  <div className="bg-red-50 dark:bg-red-900/10 p-4 rounded-2xl border border-red-100 dark:border-red-900/30"><span className="text-red-400 dark:text-red-500 font-bold uppercase tracking-wider text-[10px]">Emergency Type</span><p className="font-extrabold text-red-700 dark:text-red-400 mt-1 text-base">{currentLiveUser.emergencyType || "Emergency Alert"}</p></div>
                </div>

                <div className="bg-blue-50 dark:bg-blue-900/10 p-5 rounded-2xl border border-blue-100 dark:border-blue-900/30 space-y-3">
                  <span className="font-bold uppercase tracking-wider text-[10px] text-blue-600 dark:text-blue-400 flex items-center gap-1.5"><Navigation className="w-3 h-3"/> GPS Location Sync</span>
                  <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4">
                    <p className="font-bold text-gray-800 dark:text-slate-200 break-words flex-1 text-sm md:text-base leading-snug">{currentLiveUser.location || "Coordinates Synced"}</p>
                    <button onClick={handleLocationSearchFilter} className="bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white font-black text-xs uppercase px-6 py-3.5 rounded-xl shadow-lg shadow-blue-500/30 flex items-center justify-center gap-2 shrink-0 transition-all hover:scale-105 active:scale-95 cursor-pointer"><FaLocationDot /> Search Ambulance</button>
                  </div>
                </div>

                <div className="flex flex-col sm:flex-row gap-3 pt-2">
                  <a href={`tel:${formatPhoneNumber(currentLiveUser.phone)}`} onClick={stopAlertTone} className="flex-1 flex items-center justify-center gap-2 bg-gray-900 dark:bg-white dark:text-gray-900 hover:bg-black text-white font-bold text-sm py-3.5 rounded-xl shadow-md transition-all active:scale-95 cursor-pointer"><IoCall className="text-lg" /> Call Patient</a>
                  <button onClick={dismissRequest} className="bg-gray-100 dark:bg-slate-800 hover:bg-gray-200 dark:hover:bg-slate-700 text-gray-700 dark:text-slate-300 border border-gray-200 dark:border-slate-700 font-bold text-sm px-6 py-3.5 rounded-xl transition-all active:scale-95 cursor-pointer">Dismiss</button>
                </div>
              </>
            )}
          </div>
          ...
          <div className="flex border-b border-gray-200 dark:border-slate-800 gap-6 overflow-x-auto [&::-webkit-scrollbar]:hidden scroll-smooth whitespace-nowrap">
            {[
              { id: "live", label: "LIVE FLEET CONTROL" }, { id: "ambLogs", label: "BOOKING HISTORY" }, { id: "patientLogs", label: "USER ALERTS" }
            ].map((tab) => (
              <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`py-3 font-extrabold text-xs tracking-wider uppercase transition-all relative cursor-pointer ${activeTab === tab.id ? "text-blue-600 dark:text-blue-400 border-b-2 border-blue-600 dark:border-blue-400" : "text-gray-400 hover:text-gray-700 dark:text-slate-500 dark:hover:text-slate-300"}`}>
                {tab.label}
              </button>
            ))}
          </div>
          ...
          {activeTab === "live" && (
            <div className="space-y-6 animate-fade-in">
              <div className="flex justify-between items-center">
                <h2 className="text-lg font-black text-gray-900 dark:text-white flex items-center gap-2"><span className="h-2.5 w-2.5 rounded-full bg-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.8)] animate-pulse"></span>Active Fleet Status</h2>
              ...
              {!isLocationFiltered ? (
                <div className="bg-gradient-to-br from-indigo-50 to-blue-50 dark:from-slate-800/50 dark:to-slate-900/50 border border-indigo-100 dark:border-slate-800 rounded-3xl p-12 text-center flex flex-col items-center justify-center gap-4">
                  <div className="w-20 h-20 bg-white dark:bg-slate-800 rounded-full shadow-lg flex items-center justify-center text-indigo-500 dark:text-indigo-400 text-4xl"><MdDirectionsCar /></div>
                  <div><h3 className="text-xl font-black text-indigo-950 dark:text-white">Ambulances Hidden</h3><p className="text-sm font-medium text-indigo-600/70 dark:text-slate-400 mt-2 max-w-sm mx-auto">Click the "Search Ambulance" button on an active emergency request to reveal nearby available vehicles.</p></div>
                </div>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {displayedAmbulances.map((amb) => {
                    ...
                    const isDispatched = lastReqForAmb ? !lastReqForAmb.status.includes("Completed") && !lastReqForAmb.status.includes("Reject") && !lastReqForAmb.status.includes("Cancel") && !lastReqForAmb.status.includes("Free") : false;
                    ...
                    return (
                      <div key={amb.id} className="bg-white dark:bg-slate-900 rounded-3xl border border-gray-100 dark:border-slate-800 overflow-hidden flex flex-col transition-all duration-300 hover:shadow-2xl hover:shadow-blue-500/10 hover:-translate-y-1 relative group">
                        ...
                        <div className="h-48 w-full bg-gray-100 dark:bg-slate-800 relative overflow-hidden">
                          ...
                          <div className="absolute bottom-3 left-4 z-20"><span className="bg-white/20 backdrop-blur-md text-white text-[10px] font-black uppercase px-2.5 py-1 rounded-lg border border-white/30 tracking-wider">{amb.vehicleNumber}</span></div>
                          <button onClick={() => setDeleteTargetId(amb.id)} className="absolute top-3 right-3 z-20 bg-black/40 hover:bg-red-600 backdrop-blur-md text-white p-2 rounded-full transition-all cursor-pointer"><RiDeleteBin6Line className="w-4 h-4" /></button>
                        </div>

                        <div className="p-5 flex-1 flex flex-col justify-between">
                          <div className="space-y-4">
                            <div className="flex justify-between items-start gap-2">
                              <div><h3 className="text-lg font-black text-gray-900 dark:text-white leading-tight">{amb.driver}</h3><p className="text-xs text-gray-500 dark:text-slate-400 font-bold mt-1">{amb.type}</p></div>
                            ...
                            <div className="bg-gray-50 dark:bg-slate-800/50 rounded-xl p-3 space-y-2 border border-gray-100 dark:border-slate-700/50">
                              <p className="flex items-center gap-2 text-xs font-semibold text-gray-700 dark:text-slate-300"><MdPhone className="text-blue-500" /> {amb.phone}</p>
                              <p className="flex items-center gap-2 text-xs font-semibold text-gray-700 dark:text-slate-300"><FaLocationDot className="text-blue-500" /> Distance: <span className="text-blue-600 dark:text-blue-400 font-black">{amb.distance}</span></p>
                            </div>
                          </div>
                          ...
                          <div className="pt-4 mt-auto">
                            {isDispatched ? (
                              <div className="flex gap-2">
                                <a href={`tel:${formatPhoneNumber(amb.phone)}`} className="flex-1 flex items-center justify-center gap-2 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white text-sm font-bold py-3.5 rounded-xl shadow-md transition-all active:scale-95 cursor-pointer"><IoCall className="text-lg" /> Call Driver</a>
                                <button onClick={() => {
                                ...
                                }} className="bg-red-50 hover:bg-red-100 dark:bg-red-900/20 dark:hover:bg-red-900/40 text-red-600 border border-red-100 dark:border-red-800/50 px-4 rounded-xl font-bold text-xl transition-all cursor-pointer"><IoCloseCircle /></button>
                              </div>
                            ) : (
                              <>
                      ...
                      <div key={req.id} className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-3xl p-5 shadow-sm hover:shadow-lg transition-shadow relative overflow-hidden">
                        ...
                        <div className="pl-4">
                          <div className="flex justify-between items-start border-b border-gray-100 dark:border-slate-800 pb-3 mb-4">
                            <div><p className="text-[10px] font-black uppercase text-gray-400 dark:text-slate-500 tracking-wider">Ride ID: {req.id}</p><h3 className="text-lg font-black text-gray-900 dark:text-white mt-1">{req.vehicleNumber || "RJ-18-AMB"}</h3></div>
                            <div className="text-right"><span className="bg-gray-100 dark:bg-slate-800 text-gray-600 dark:text-slate-300 text-[10px] font-bold px-2.5 py-1 rounded-lg">{req.time}</span></div>
                          </div>
                          ...
                          <div className="flex items-center justify-between mt-2 pt-4 border-t border-gray-100 dark:border-slate-800">
                            ...
                            {isCompleted ? (
                              <div className="flex items-center gap-1.5 text-emerald-600 dark:text-emerald-400 font-black text-sm bg-emerald-50 dark:bg-emerald-900/20 px-4 py-2 rounded-xl"><CheckCircle2 className="w-4 h-4" /> COMPLETED</div>
                            ) : (
                              <button onClick={() => setTrackingData(req)} className="bg-gray-900 dark:bg-white text-white dark:text-gray-900 hover:scale-105 transform transition-all text-xs font-bold px-5 py-2.5 rounded-xl flex items-center gap-2 shadow-md cursor-pointer"><Navigation className="w-3.5 h-3.5" /> Track Ride</button>
                            )}
                          </div>
                        </div>
                      </div>
            ...
            <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-fade-in">
              <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-2xl border border-gray-200 dark:border-slate-700 max-w-2xl w-full p-6 space-y-5">
                <div className="flex items-center justify-between border-b border-gray-100 dark:border-slate-800 pb-4">
                  <h3 className="text-lg font-black text-gray-900 dark:text-white flex items-center gap-2"><Navigation className="text-blue-500"/> Live Tracking</h3>
                  <button onClick={() => setTrackingData(null)} className="text-gray-400 hover:text-red-500 bg-gray-100 dark:bg-slate-800 rounded-full p-1.5 transition-colors cursor-pointer"><IoCloseCircle className="text-2xl" /></button>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-blue-50 dark:bg-blue-900/10 p-4 rounded-2xl border border-blue-100 dark:border-blue-900/30">
                    ...
                    <p className="font-black text-gray-900 dark:text-white">{trackingData.driverName}</p><p className="text-gray-500 dark:text-slate-400 text-[10px] mt-0.5">{trackingData.vehicleNumber || "RJ-18-AMB"}</p>
                  </div>
                  <div className="bg-emerald-50 dark:bg-emerald-900/10 p-4 rounded-2xl border border-emerald-100 dark:border-emerald-900/30">
                    ...
                    <p className="font-black text-gray-900 dark:text-white">{trackingData.patientName}</p><p className="text-gray-500 dark:text-slate-400 text-[10px] mt-0.5 truncate">{trackingData.location}</p>
                  </div>
                </div>
                <div className="w-full h-96 bg-gray-100 dark:bg-slate-800 rounded-2xl overflow-hidden border border-gray-200 dark:border-slate-700 shadow-inner">
                  <iframe title="Live Route" src={`https://maps.google.com/maps?saddr=${trackingData.ambLat},${trackingData.ambLng}&daddr=${trackingData.userLat},${trackingData.userLng}&output=embed`} className="w-full h-full" allowFullScreen loading="lazy"/>
                </div>
              </div>
            </div>
          )}
      ...
      <style>{`@keyframes fadeIn { from { opacity: 0; } to { opacity: 1; } } @keyframes slideDown { from { transform: scaleY(0); opacity: 0; } to { transform: scaleY(1); opacity: 1; } } .animate-fade-in { animation: fadeIn 0.3s ease-out forwards; } .animate-slide-down { transform-origin: top; animation: slideDown 0.2s ease-out forwards; }`}</style>
    </div>
  );
}