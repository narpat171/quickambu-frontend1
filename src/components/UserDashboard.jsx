  ...
  useEffect(() => {
    socket.on("journey-status-updated", (data) => {
      if (data.reqId === myReqId || data.reqId === localStorage.getItem("myReqId")) {
        ...
        // ...
        if (data.step === 7) {
          // आपका पुराना Trip Completed कोड
        } else if (data.step === -1) {
          // 👈 BUG FIX: ड्राइवर रिजेक्ट करे तो यूज़र का पैनल रिसेट करो
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-[9999] p-4 backdrop-blur-sm">
          <div className="bg-white rounded-2xl max-w-sm w-full p-6 shadow-2xl text-center transform transition-all scale-100">
            ...
            <div className="flex justify-center space-x-3">
              ...
              <button onClick={() => { modalConfig.onConfirm(); setShowModal(false); }} className={`px-6 py-3 font-bold rounded-xl text-white w-full shadow-md ${modalConfig.title.includes('🚨') || modalConfig.title.includes('Error') ? 'bg-red-600 hover:bg-red-700' : 'bg-emerald-600 hover:bg-emerald-700'}`}>OK</button>
            </div>
          </div>
        </div>
      )}
